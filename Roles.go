...
func Roles(c *gin.Context) {
	...
	if helper.ValidateHeader(bodyString, c) {
		if err := c.ShouldBindJSON(&jRolesRequest); err != nil {
			...
			RoleName := jRolesRequest.RoleName
			...
			if Method == "INSERT" {
				...
				if err := db.QueryRow(query).Scan(&totalRecords); err != nil {
					...
					if totalRecords > 0 {
						...
						newID := 0
						queryGetID := fmt.Sprintf("SELECT IFNULL(max(id),0) + 1 as newID  FROM siam_role ")
						if err := db.QueryRow(queryGetID).Scan(&newID); err != nil {
							errorMessage := "Error query, " + err.Error()
							returnDataJsonRoles(jRolesResponses, totalPage, "1", "1", errorMessage, errorMessage, logData, c)
							helper.SendLogError(Username, PageGo, errorMessage, "", "", "1", AllHeader, Method, Path, IP, c)
							return
						} else {
							queryInsert := fmt.Sprintf("INSERT INTO siam_role (id, nama_role, tgl_input) values ('%d','%s',sysdate())", newID, RoleName)
							//insertResult, err := db.Exec( queryInsert)
				...
				if RoleName != "" {
					...
					queryCek += " nama_role  = upper('" + RoleName + "') "
				}

				if queryCek != "" {
					queryCek = " WHERE " + queryCek
				}

				// --------- End of query Check Role ------------
				totalRecords = 0
				query := fmt.Sprintf("SELECT COUNT(1) AS cnt FROM siam_role %s", queryCek)
				...
				if err := db.QueryRow(query).Scan(&totalRecords); err != nil {
					...
					if totalRecords > 0 {
						...
						queryUpdate := fmt.Sprintf("UPDATE siam_role  %s %s", querySet, queryWhere)
						//insertResult, err := db.Exec( queryInsert)
				...
				if queryWhere != "" {
					...
					errorMessage := "Parameter can not be null!"
					returnDataJsonRoles(jRolesResponses, totalPage, "3", "3", errorMessage, errorMessage, logData, c)
					helper.SendLogError(Username, PageGo, errorMessage, "", "", "3", AllHeader, Method, Path, IP, c)
					return
				}
				...
				if err := db.QueryRow(query).Scan(&totalRecords); err != nil {
					...
					if totalRecords == 1 {
						...
						queryInsert := fmt.Sprintf("DELETE FROM siam_role %s", queryWhere)
						//insertResult, err := db.Exec( queryInsert)
						if _, err := db.Exec(queryInsert); err != nil {
							errorMessage := "Error insert, " + err.Error()
							returnDataJsonRoles(jRolesResponses, totalPage, "1", "1", errorMessage, errorMessage, logData, c)
				...
				query1 := fmt.Sprintf(`SELECT id, nama_role, tgl_input FROM siam_role %s %s LIMIT %d,%d;`, queryWhere, queryOrder, PageNow, RowPage)
				fmt.Println(query1)
